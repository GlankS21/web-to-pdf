...
const SignUpPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    setError(null)
    await signUp(form.username, form.password, form.email, form.firstName, form.lastName)
    const { loading } = useAuthStore.getState()
    if (!loading) navigate('/signin')
  }
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .auth-header {
          ...
          display: flex; align-items: center; height: 56px; background: var(--surface);
        }
        .auth-logo {
        ...
        .auth-title { font-size: 24px; font-weight: 800; letter-spacing: -0.02em; margin-bottom: 4px; }
        .auth-sub { font-size: 12px; font-family: 'JetBrains Mono', monospace; color: var(--muted); margin-bottom: 32px; }
        ...
        .auth-footer { margin-top: 24px; text-align: center; font-size: 12px; font-family: 'JetBrains Mono', monospace; color: var(--muted); }
        .auth-footer a { color: var(--accent); text-decoration: none; }
        .auth-footer a:hover { text-decoration: underline; }
        .strength-bar { height: 3px; border-radius: 2px; margin-top: 6px; transition: all 0.2s; background: var(--border); }
        .strength-bar.weak { background: var(--accent2); width: 33%; }
        .strength-bar.medium { background: #f5a623; width: 66%; }
        .strength-bar.strong { background: var(--accent); width: 100%; }
      ...
      <div className="auth-root">
        ...
        <div className="auth-body">
          <div className="auth-card">
            ...
            <div className="auth-sub">— set up your workspace</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div className="auth-field">
                ...
                <input className="auth-input" value={form.firstName} onChange={e => setForm(p => ({ ...p, firstName: e.target.value }))} />
              </div>
              <div className="auth-field">
                <label className="auth-label">Last name</label>
                <input className="auth-input" value={form.lastName} onChange={e => setForm(p => ({ ...p, lastName: e.target.value }))} />
              </div>
            </div>
            ...
            <div className="auth-field">
              ...
              <input
                ...
                placeholder="••••••••"
                autoComplete="new-password"
                style={{ borderColor: form.confirm && form.confirm !== form.password ? 'var(--accent2)' : '' }}
              />
            </div>
